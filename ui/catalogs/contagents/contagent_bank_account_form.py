...
class ContagentBankAccountForm(QDialog):
    def __init__(
        ...
        layout.addWidget(QLabel("Номер счёта"))  # IBAN/р/с
        self.acc_input = QLineEdit()
        self.acc_input.setMaxLength(34)
        layout.addWidget(self.acc_input)

        layout.addWidget(QLabel("Валюта (опционально)"))  # RUB/ABH/USD...
    ...
    def _load(self) -> None:
        self.acc_input.setText(self.account.account_number or "")
        self.currency_input.setText(self.account.currency or "")
        self.note_input.setText(self.account.note or "")
        # select bank
        idx = self.bank_combo.findData(int(self.account.bank_id))
        if idx >= 0:
    ...
    def _save(self) -> None:
        ...
        try:
            if self.account is None:
                ...
                update_bank_account(int(self.account.id), data)
