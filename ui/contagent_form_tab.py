...
class ContagentFormTab(QWidget):
    """
    Форма контрагента как вкладка (как в 1С).
    Кнопки: Записать / OK / Отмена.
    """
    def __init__(
        self,
        main_window: Any,
        *,
        ...
        # ключ вкладки нужен, чтобы закрывать себя через TabManager
        self.tab_key = tab_key or f"contagent:{uuid.uuid4().hex[:8]}"
        ...
        if self.contagent_id is not None and self.mode in ("edit", "copy"):
            self._load_data()
    ...
    def _init_ui(self) -> None:
        ...
        self.ok_btn.clicked.connect(self._save_and_close_tab)
    ...
    def _load_data(self) -> None:
        if self.contagent_id is None:
            return
        c = get_contagent_by_id(self.contagent_id)
        if not c:
            self._status("Контрагент не найден", "error", 5000)
            return

        self.contagent = c

        # copy: данные загрузили, но сохраняем как нового
        if self.mode == "copy":
            self.contagent_id = None
        ...
        self.active_checkbox.setChecked(bool(c.is_active))
        self.deleted_checkbox.setChecked(bool(c.is_deleted))
    ...
    def _validate(self) -> bool:
        ...
        if not name:
            self._status("Название обязательно", "error", 5000)
            return False
        ...
        if inn and len(inn) != 8:
            self._status("ИНН должен содержать 8 цифр", "error", 5000)
            return False

        if kpp and len(kpp) != 9:
            self._status("КПП должен содержать 9 цифр", "error", 5000)
            return False
    ...
    def _save(self) -> bool:
        # если edit и нет изменений — OK просто закроет вкладку
        ...
        try:
            ...
            self._status(str(e), "error", 7000)
            return False

        self.is_modified = False
        self._status("Контрагент сохранён", "success", 3000)
    ...
    def _save_and_close_tab(self) -> None:
        if self._save():
            self._close_tab()
