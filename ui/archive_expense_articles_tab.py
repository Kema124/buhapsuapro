...
from services.expense_articles import (
    get_deleted_expense_articles, restore_expense_article, delete_expense_articles_forever
)


class ArchiveExpenseArticlesTab(QWidget):
    ...
    def load_data(self) -> None:
        data = get_deleted_expense_articles()
    ...
    def restore_selected(self) -> None:
        ...
        for aid in ids:
            try:
                restore_expense_article(aid)
                ok += 1
            except Exception as e:
                self._msg(f"Не удалось восстановить {aid}: {e}", "error", 6000)
        self.load_data()
    ...
    def delete_forever_selected(self) -> None:
        ...
        try:
            delete_expense_articles_forever(ids)
            self.load_data()
