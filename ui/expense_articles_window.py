...
from services.expense_articles import (
    get_all_expense_articles, search_expense_articles,
    filter_expense_articles, soft_delete_expense_article
)
...
class ExpenseArticlesWindow(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self.search_input.setPlaceholderText("Поиск: наименование / группа")
        ...
        self.table.setHorizontalHeaderLabels(["ID", "Наименование", "Группа", "Комментарий"])
        ...
        self.btn_copy.clicked.connect(lambda: self._msg("Копирование статей — позже", "info", 3500))
        self.btn_add.clicked.connect(lambda: self._msg("Создание статей — позже", "info", 3500))
    ...
    def _on_filter_applied(self, filters: dict) -> None:
        self._fill(filter_expense_articles(filters))
    ...
    def archive_selected(self) -> None:
        ...
        if not ids:
            self._msg("Выберите статьи расходов", "warning", 2500)
            return
        ok = 0
        for aid in ids:
            try:
                soft_delete_expense_article(aid)
                ok += 1
            except Exception as e:
                self._msg(f"Не удалось архивировать {aid}: {e}", "error", 6000)
        self.load_data()
    ...
    def edit_selected(self) -> None:
        ids = self.selected_ids()
        if not ids:
            self._msg("Выберите статью", "warning", 2500)
            return
        self._msg("Форма статьи расходов — на следующем этапе", "info", 3500)
    ...
    def _menu(self, pos: QPoint) -> None:
        ...
        if act == a_add:
            self._msg("Создание статей — позже", "info", 3500)
        elif act == a_open:
            self.edit_selected()
        elif act == a_copy:
            self._msg("Копирование статей — позже", "info", 3500)
        elif act == a_arch:
