...
def get_all_banks() -> list[Bank]:
    ...
    try:
        return (
            session.query(Bank)
            .filter(Bank.is_deleted.is_(False))
            .order_by(Bank.name)
            .all()
        )
    finally:
...
def get_deleted_banks() -> list[Bank]:
    ...
    try:
        return (
            session.query(Bank)
            .filter(Bank.is_deleted.is_(True))
            .order_by(Bank.name)
            .all()
        )
    finally:
...
def update_bank(bank_id: int, data: dict[str, Any]) -> None:
    session = db.get_session()
    try:
        obj = session.query(Bank).filter(Bank.id == bank_id, Bank.is_deleted.is_(False)).first()
        if not obj:
...
def search_banks(query: str) -> list[Bank]:
    session = db.get_session()
    try:
        q = session.query(Bank).filter(Bank.is_deleted.is_(False))
