...
def get_all_taxes() -> list[Tax]:
    ...
    try:
        return (
            session.query(Tax)
            .filter(Tax.is_deleted.is_(False))
            .order_by(Tax.name)
            .all()
        )
    finally:
...
def get_deleted_taxes() -> list[Tax]:
    ...
    try:
        return (
            session.query(Tax)
            .filter(Tax.is_deleted.is_(True))
            .order_by(Tax.name)
            .all()
        )
    finally:
...
def update_tax(tax_id: int, data: dict[str, Any]) -> None:
    session = db.get_session()
    try:
        obj = session.query(Tax).filter(Tax.id == tax_id, Tax.is_deleted.is_(False)).first()
        if not obj:
...
def search_taxes(query: str) -> list[Tax]:
    session = db.get_session()
    try:
        q = session.query(Tax).filter(Tax.is_deleted.is_(False))
