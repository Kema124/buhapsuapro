...
class BankReceiptForm(QDialog):
    ...
    def _load_contracts(self) -> None:
        ...
        if cid is None:
            return
        cid = self._safe_int(self.contagent.currentData())
        if cid is None:
            return

        for ctr in get_contracts_by_contagent(cid):
            ctr_id = self._safe_int(getattr(ctr, "id", None))
            if ctr_id is None:
                continue
            self.contract.addItem(f"{ctr.number} от {ctr.date}", ctr_id)
    ...
    def _post(self) -> None:
        ...
        if self.receipt_id is None:
            self._msg("Не удалось сохранить документ", "error", 5000)
            return

        try:
            post_bank_receipt(self.receipt_id)
        except Exception as e:
    ...
    def _unpost(self) -> None:
        ...
        try:
            unpost_bank_receipt(receipt_id)
        except Exception as e:
